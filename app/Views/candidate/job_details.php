...
$showInterviewNudge = !$isExternalJob && !$alreadyApplied && !$isExpired && $policyRaw !== 'OFF';
$interviewNudge = [
    ...
    'video_url'   => base_url('videos/interview_demo.mp4'),
...
<div id="aiNudge-backdrop" role="dialog" aria-modal="true" aria-label="AI Interview practice suggestion">
  <div id="aiNudge-modal">
    <div class="ain-header">
      <div class="ain-header-left">
        <span class="ain-eyebrow"><?= esc($interviewNudge['eyebrow']) ?></span>
        <span class="ain-counter">Application submitted for <?= esc($job['title']) ?></span>
      </div>
      <button class="ain-close-x" id="aiNudgeCloseX" aria-label="Close">✕</button>
    </div>

    <div class="ain-body">
      <h2 class="ain-slide-title"><?= esc($interviewNudge['title']) ?></h2>
      ...
      <ul class="ain-rows">
        <?php foreach ($interviewNudge['rows'] as $row): ?>
          <li><i class="fas fa-check-circle"></i><?= esc($row) ?></li>
        <?php endforeach; ?>
      </ul>
    </div>

    <div class="ain-divider"></div>

    <div class="ain-footer">
      <a href="<?= esc($interviewNudge['cta_url']) ?>" class="btn btn-primary">
        <i class="fas fa-magic"></i> <?= esc($interviewNudge['cta_label']) ?>
      </a>
      <button class="ain-btn-maybe" id="aiNudgeClose">Maybe later</button>
    </div>
  </div>
</div>
...
<script>
(function () {
  var backdrop = document.getElementById('aiNudge-backdrop');
  if (!backdrop) return;
  ...
  // Show once per job, on first page load only, per browser session.
  var storageKey = 'aiNudgeShown_<?= (int) $job['id'] ?>';
  var alreadyShown = false;
  try {
    alreadyShown = sessionStorage.getItem(storageKey) === '1';
  } catch (e) {
    alreadyShown = false; // sessionStorage unavailable (e.g. private mode) — fail open
  }
...
if (!alreadyShown) {
  ...
  document.getElementById('aiNudgeClose').addEventListener('click', closeNudge);
  document.getElementById('aiNudgeCloseX').addEventListener('click', closeNudge);
