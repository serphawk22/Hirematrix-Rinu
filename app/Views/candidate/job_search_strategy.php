...
if (empty($jobsWorthApplying)) {
    $jobsWorthApplying = array_values(array_filter($topSuggestedJobs, static function (array $job) use ($recommendedIds): bool {
        return in_array((int) ($job['id'] ?? 0), $recommendedIds, true);
    }));
}
if (empty($jobsWorthApplying)) {
    $jobsWorthApplying = array_slice($topSuggestedJobs, 0, 3);
}
...
$profileBlockers = array_values(array_filter(array_map('trim', (array) ($jobSearchStrategy['profile_blockers'] ?? $profileFixes))));
...
$formatSkill = static function (string $skill): string {
    $skill = trim($skill);
    return $skill === '' ? '' : ucwords($skill);
};

$roadmapPhases = [
    [
        'step' => '01',
        'title' => 'Best Target Roles',
        'copy' => 'Keep applications centered on these role titles first.',
        'items' => $targetRoles,
    ],
    [
        'step' => '02',
        'title' => 'Profile Gaps Blocking Matches',
        'copy' => 'Fix these before spending effort on weaker applications.',
        'items' => $profileBlockers,
    ],
    [
        'step' => '03',
        'title' => 'Jobs Worth Applying To',
        'copy' => 'Prioritize role-relevant jobs with stronger match signals.',
        ...
        <div class="strategy-shell">
            <aside class="strategy-sidebar">
                <div class="strategy-sidebar-block">
                    ...
                    <p class="text-muted mb-0">
                        This plan filters <?= count($jobsWorthApplying) ?> jobs worth applying to, <?= count($jobsToAvoid) ?> jobs to avoid, and <?= count($profileBlockers) ?> blockers to fix.
                    </p>
                </div>

                <div class="strategy-sidebar-block">
                    <h3>Best Target Roles</h3>
            ...
            <div class="strategy-main">
                <section class="strategy-card strategy-hero-card">
                    ...
                    <p class="strategy-summary"><?= esc($jobSearchStrategy['summary'] ?? 'Use this plan to choose stronger applications, close blocking profile gaps, and skip jobs that do not fit your target role.') ?></p>
                </section>

                <section class="strategy-section">
                    <div class="strategy-section-heading">
                        <div>
                            <h3>Practical Search Plan</h3>
                            <p>Use these sections to decide where to apply, what to fix, and what to skip.</p>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-lg-6 mb-4 mb-lg-0">
                        <section class="strategy-card h-100">
                            <div class="strategy-section-title">Profile Gaps Blocking Matches</div>
                            <ul class="strategy-list strategy-plan-list">
                                <?php foreach (array_slice($profileBlockers, 0, 5) as $item): ?>
                                    <li><?= esc($item) ?></li>
                                <?php endforeach; ?>
                            </ul>
                        </section>
                    ...
                    <div class="col-lg-6">
                        <section class="strategy-card h-100">
                            <div class="strategy-section-title">Jobs To Avoid</div>
                ...
                <?php if (!empty($jobsWorthApplying)): ?>
                    <section class="strategy-card">
                        <div class="strategy-section-title">Jobs Worth Applying To</div>
                        <div class="strategy-job-list">
                            <?php foreach ($jobsWorthApplying as $job): ?>
                                <?php
                                $missingSkills = array_values(array_filter((array) ($job['missing_skills'] ?? [])));
                                $matchedSkills = array_values(array_filter((array) ($job['matched_skills'] ?? [])));
                                ?>
                                <article class="strategy-job">
                                    ...
                                    <div class="strategy-job-details">
                                        <div class="strategy-job-note">
                                            <strong>Why this role</strong>
                                            <span><?= esc($job['why_this_role'] ?? ($job['match_reason'] ?? 'This job has useful overlap with your current profile.')) ?></span>
                                        </div>
                                        <div class="strategy-job-note">
                                            <strong>What to fix before applying</strong>
                                            <span><?= esc($job['fix_before_applying'] ?? 'Tailor your resume headline, summary, and strongest project to this job before applying.') ?></span>
                                        </div>
                                        <div class="strategy-job-skill-row">
                                            <span class="strategy-job-skill-label">Matched skills</span>
                                            <?php if (!empty($matchedSkills)): ?>
                                                <?php foreach (array_slice($matchedSkills, 0, 5) as $skill): ?>
                                                    <span class="strategy-chip strategy-chip-compact"><?= esc($formatSkill((string) $skill)) ?></span>
                                                <?php endforeach; ?>
                                            <?php else: ?>
                                                <span class="text-muted">No clear skill overlap listed.</span>
                                            <?php endif; ?>
                                        </div>
                                        <div class="strategy-job-skill-row">
                                            <span class="strategy-job-skill-label">Missing skills</span>
                                            <?php if (!empty($missingSkills)): ?>
                                                <?php foreach (array_slice($missingSkills, 0, 5) as $skill): ?>
                                                    <span class="strategy-chip strategy-chip-compact is-missing"><?= esc($formatSkill((string) $skill)) ?></span>
                                                <?php endforeach; ?>
                                            <?php else: ?>
                                                <span class="text-muted">No major missing skills from this listing.</span>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </article>
                ...
                <?php if (!empty($jobsToAvoid)): ?>
                    <section class="strategy-card">
                        <div class="strategy-section-title">Jobs To Avoid</div>
                        <div class="strategy-job-list">
                            <?php foreach ($jobsToAvoid as $job): ?>
                                <?php $missingSkills = array_values(array_filter((array) ($job['missing_skills'] ?? []))); ?>
                                <article class="strategy-job strategy-job-avoid">
                                    <div class="strategy-job-top">
                                        <div>
                                            <div class="strategy-job-title">
                                                <a href="<?= base_url('job/' . (int) $job['id']) ?>"><?= esc($job['title'] ?? 'Untitled Role') ?></a>
                                        ...
                                        <span class="strategy-avoid-label">Lower priority</span>
                                    </div>
                                    <div class="strategy-job-details">
                                        <div class="strategy-job-note">
                                            <strong>Why avoid for now</strong>
                                            <span><?= esc($job['avoid_reason'] ?? 'This role is less aligned with the target roles and stronger matches available now.') ?></span>
                                        </div>
                                        <div class="strategy-job-note">
                                            <strong>Apply later if</strong>
                                            <span><?= esc($job['apply_later_if'] ?? 'Reconsider only after stronger-fit jobs are handled or your target role changes.') ?></span>
                                        </div>
                                        <div class="strategy-job-skill-row">
                                            <span class="strategy-job-skill-label">Missing skills</span>
                                            <?php if (!empty($missingSkills)): ?>
                                                ...
                                                <?php endforeach; ?>
                                            <?php else: ?>
                                                <span class="text-muted">The main issue is role/title relevance, not skills.</span>
                                            <?php endif; ?>
                                        </div>
                                        <div class="strategy-job-actions">
                                            <a href="<?= base_url('job/' . (int) $job['id']) ?>" class="btn btn-outline-secondary btn-sm">Review Anyway</a>
                                        </div>
                                    </div>
                                </article>
                            <?php endforeach; ?>
