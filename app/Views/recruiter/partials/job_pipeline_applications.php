...
(function () {
    ...
    document.addEventListener('click', function (event) {
        var toggle = event.target.closest('.hm-status-drop-btn');
        if (toggle) {
            event.preventDefault();
            event.stopPropagation();
            var menu = toggle.closest('.hm-status-drop').querySelector('.hm-status-drop-menu');
            document.querySelectorAll('.hm-status-drop-menu').forEach(function (item) {
        ...
        var item = event.target.closest('.hm-pipeline-status-change');
        if (!item) {
            document.querySelectorAll('.hm-status-drop-menu').forEach(function (menu) {
        ...
        event.preventDefault();
        event.stopPropagation();
        var appId = item.dataset.applicationId;
        var status = item.dataset.status;
        var label = item.dataset.label;
        var drop = item.closest('.hm-status-drop');
        var button = drop ? drop.querySelector('.hm-status-drop-btn') : null;
        var base = page.dataset.statusUrlBase || '';
        var url = base + appId;

        if (status === 'shortlisted') {
            url = base.replace('update-status/', 'shortlist/') + appId;
        } else if (status === 'rejected') {
            url = base.replace('update-status/', 'reject/') + appId;
        }
        ...
        fetch(url, {
            ...
            .then(function (payload) {
                if (payload && payload.csrf_hash && payload.csrf_token_name) {
                    page.dataset.csrfHash = payload.csrf_hash;
                    document.querySelectorAll('input[name="' + payload.csrf_token_name + '"]').forEach(function (input) {
                        input.value = payload.csrf_hash;
                    });
                }
                if (button) {
